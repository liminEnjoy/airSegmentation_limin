...
config = {}
config['anchors'] = [5.0, 10.0, 20.0, 40.0, 60.0, 80.0, 100.0]
config['chanel'] = 1
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size = 1),
                                    nn.ReLU(),
                                    #nn.Dropout3d(p = 0.3),
                                   nn.Conv3d(64, 9 * len(config['anchors']), kernel_size = 1))
    ...
    def forward(self, x, coord):
        ...
        out = out.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], len(config['anchors']), 9)
        #out = out.view(-1, 5)
        return out
