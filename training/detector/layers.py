...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True):
        ...
        pos_idcs = pos_idcs.unsqueeze(1).expand(pos_idcs.size(0), 9)
        pos_output = output[pos_idcs].view(-1, 9)
        pos_labels = labels[pos_idcs].view(-1, 9)
        ...
        if len(pos_output)>0:
            ...
            classify_loss = self.classify_loss(torch.cat((pos_prob,neg_prob),0), torch.cat((pos_labels[:, 0],neg_labels + 1),0)) + self.ce_loss(pos_output[:,4:],torch.argmax(pos_labels[:,4:],dim=1))
