...
parser.add_argument('--save-dir', default='res18_extend', type=str, metavar='SAVE',
...
def main():
    ...
    net.load_state_dict(torch.load("/data/wzeng/DSB_3/training/detector/results/res18_extend/048.ckpt")['state_dict'])
...
#         name = data_loader.dataset.filenames[i_name].split('-')[0].split('/')[-1].split('_clean')[0]
        ...
        thresh = -3
        pbb,mask,bboxes = get_pbb(output,thresh,ismask=True)
        for bbox in bboxes:
            x = bbox[1]
            y = bbox[2]
            z = bbox[3]
            d = bbox[4]
            temp = [bbox[5],bbox[6],bbox[7],bbox[8]]
            p = max(softmax(temp))
            c =np.argmax(softmax(temp))+1
            csv_write.writerow([name,p,x,y,z,d,c])
        if isfeat:
            feature_selected = feature[mask[0],mask[1],mask[2]]
            np.save(os.path.join(save_dir, name+'_feature.npy'), feature_selected)
        #tp,fp,fn,_ = acc(pbb,lbb,0,0.1,0.1)
        #print([len(lbb),len(tp),len(fp),len(fn)])
        #print([i_name,name])
    #     e = time.time()
