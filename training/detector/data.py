...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        ...
        idcs = np.load(split_path) # split_path contains the names of cts
        # delete the id in the self.blacklist
        if phase!='test':
            idcs = [f for f in idcs if (f not in self.blacklist)]
        
        self.filenames = [os.path.join(data_dir, '%s_clean.npy' % idx) for idx in idcs]
        self.kagglenames = [os.path.join(data_dir, '%s_clean.npy' % idx) for idx in idcs]
        self.lunanames = [os.path.join(data_dir, '%s_clean.npy' % idx) for idx in idcs]
        ...
        for idx in idcs:
#             print(idx)
            lb = np.load(os.path.join(data_dir, '%s_label.npy' %idx))
            ctanno = np.load(os.path.join(data_dir, '%s_ctanno.npy' %idx))
    
            # the content of label.npy is changed, label/spacing/origin
            spacing = ctanno[0]
            origin = ctanno[1]
            # extendbox = ctanno[2]
             
            # if np.all(l==0),l-->np.array([]), else, transform the world coordinate
            # to the voxel coordinage
            l = lb.copy          
            if np.all(l==0):
                l=np.array([])
            else:
            ...
            labels.append(l)
            #extendboxes.append(extendbox)

        self.sample_bboxes = labels
        self.sample_spacing = spacings
        self.sample_origin = origins
        # self.sample_extendbox = extendboxes
        if self.phase != 'test':
            self.bboxes = []
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                # resample the imgs
                spacing = self.sample_spacing[int(bbox[0])]
                imgs = resample(imgs,spacing,order=1)[0][np.newaxis,...]
                ...
                # resample the imgs            
                spacing = self.sample_spacing[randimid]
                imgs = resample(imgs,spacing,order=1)[0][np.newaxis,...]
            ...
            # resample the imgs            
            spacing = self.sample_spacing[idx]
            imgs = resample(imgs,spacing,order=1)[0][np.newaxis,...]
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes, name):
        stride = self.stride
        ...
        for i in range(3):
            if input_size[i] != 128:
                print(name,input_size[i])
            assert(input_size[i] % stride == 0)
            output_size.append(int(input_size[i] / stride))
        #print(input_size)
        
        label = -1 * np.ones(output_size + [len(anchors), 9], np.float32)
        ...
        #print(list(np.eye(4)[int(target[4])-1]))
        label[pos[0], pos[1], pos[2], pos[3], :] = [1, dz, dh, dw, dd] + list(np.eye(4)[int(target[4])-1])
        return label        
