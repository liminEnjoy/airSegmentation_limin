...
def seg_lung(path, name):
    patient = os.path.join(path,name+'.mhd')

    sliceim,origin,spacing,isflip  = load_itk_image(patient)

    bw = binarize_per_slice(sliceim, spacing)
    flag = 0
    cut_num = 0
    cut_step = 2
    bw0 = np.copy(bw)
    ...
    return sliceim, bw1, bw2, spacing, origin
...
def resample(imgs, spacing, new_spacing=np.array([1,1,1]),order=2):
    if len(imgs.shape)==3:
...
def savenpy(id,annos,filelist,data_path,prep_folder):        
    ...
    im, m1, m2, spacing, origin = seg_lung(data_path,name)
    Mask = m1+m2
    
#     newshape = np.round(np.array(Mask.shape)*spacing/resolution)
    newshape = np.round(np.array(Mask.shape))
...
#     sliceim1,_ = resample(sliceim,spacing,resolution,order=1)
    sliceim = sliceim[extendbox[0,0]:extendbox[0,1],
...
#     sliceim = sliceim2[np.newaxis,...]
    ...
    label = list(np.expand_dims(label, 0))
    label.append(spacing)
    label.append(origin)
    label.append(extendbox[:,0])
...
if __name__ == '__main__':
    ...
    prep_folder = '/home/xgz/main/DSB/data2/stage1/preprocess'
    data_path = '/home/xgz/main/DSB/data2/trainmhd'
    alllabelfiles = '/home/xgz/main/DSB/data2/csv/train/annotations.csv'
